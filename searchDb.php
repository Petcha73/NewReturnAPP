...
<?php
	...
	$search= isset($_POST['search']) ? $mysqli->real_escape_string(htmlspecialchars($_POST['search'])) : "";
	

		
		
		//create a sql query to search the string in database
		$query = "SELECT id,ticket,module,issue1,guarenty,remark,agent,dateIn,solution,trackOut,status  FROM `$dbTable` WHERE 
		`id`		 LIKE '%$search%' OR 
...
<body>
	...
	<form id ="form" action="searchDb.php" method="post">
		...
		<div id="topSearchRow" >
			...
			<img src="img/petitlogo.png">
			<INPUT id ="PageSearchBox" type="text" name="search" autofocus="autofocus" value="<?php echo $search; ?>"/>
			<INPUT id="indexPageButton" type="submit" value="Search">
			<INPUT id="indexPageButton" value="Create a new return" TYPE="button" onClick="parent.location = 'http://<?php echo $webAdress["ip"]; ?>/<?php echo $webAdress["dir"]; ?>/createReturn.php'">
			<INPUT id="indexPageButton" value="Statistics" TYPE="button" onClick="parent.location = 'http://<?php echo $webAdress["ip"]; ?>/<?php echo $webAdress["dir"]; ?>/stat.php'">
		</div>
	...
	<?php   
		...
		if ($result->num_rows > 0) { 
			...
			<th data-sort="string">	Guarenty </th>
			<th data-sort="string"> Remark </th>
			<th data-sort="string"> Agent </th>
			<th data-sort="string"> Date In </th>
			<th data-sort="string"> Solution </th>
			<th data-sort="string"> Track out </th>
			...
			while($row = $result->fetch_assoc()) {
				...
				switch($row["status"]){
					...
					case "Recieved": echo'<tr class="trReception" >';	break;
					case "Refunded": echo'<tr class="trDone" >';	break;
					case "Ready": echo'<tr class="trEnCours" >';	break;
