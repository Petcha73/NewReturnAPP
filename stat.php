...
<?php
	...
	If ( isset($month)){
		...
		$queryModule="SELECT `module`,COUNT(`id`) FROM `$dbTable` WHERE month(`dateIn`)=".$month." AND year(`dateIn`)=2016  AND `Issue1`!= 'Refund' GROUP BY `module`";
		$resultModule = $mysqli-> query($queryModule);
		
		$queryIssue="SELECT  `module`,`issue1`,COUNT(`id`) FROM `$dbTable` WHERE month(`dateIn`)=".$month." AND year(`dateIn`)=2016 GROUP BY `module`";
		$resultIssue = $mysqli-> query($queryIssue);
		
		$dateReturn="SELECT COUNT(`id`) FROM `$dbTable` WHERE month(`dateReturn`)=".$month." AND year(`dateReturn`)=2016 AND `Issue1`!= 'Refund'";
		$resultDateReturn = $mysqli-> query($dateReturn);
		
		$dateIn="SELECT COUNT(`id`) FROM `$dbTable` WHERE month(`dateIn`)=".$month." AND year(`dateIn`)=2016 AND `Issue1`!= 'Refund'";
		$resultDateIn = $mysqli-> query($dateIn);
		
		$dateOut="SELECT COUNT(`id`) FROM `$dbTable` WHERE month(`dateOut`)=".$month." AND year(`dateOut`)=2016 AND `Issue1`!= 'Refund'";
...
<body>
	...
	<span id=mainTitle > <?php echo $monthName." 2016"; ?></span>
